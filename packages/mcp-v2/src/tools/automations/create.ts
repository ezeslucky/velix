...
export function register(server: McpServer): void {
	defineTool(server, {
		...
		description:
			"Schedule a recurring agent run. Provide an RFC 5545 RRULE body for the schedule. Either v2ProjectId (run in a fresh workspace) or v2WorkspaceId (reuse an existing workspace) is required — call projects_list or workspaces_list first to get IDs. `agent` is the host-agent instance id (or presetId fallback) that runs the prompt; pass 'velix' for the built-in chat agent.",
		inputSchema: {
			name: z
				.string()
				.min(1)
			...
			agent: z
				...
				.max(200)
				.describe(
					"Host agent instance id (UUID from /settings/agents) or presetId (e.g. 'claude', 'codex'). Use 'velix' for the built-in chat agent.",
				),
			targetHostId: z
				.string()
				.min(1)
