...
export function getSyntheticPrFetchRef(prNumber: number): string {
	return `refs/velix/pr-fetch/${prNumber}/head`;
}
	...
	return {
		...
		warning:
			...
			(args.pr.isCrossRepository && !forkRemoteUrl
				? `Velix checked out PR #${args.pr.number} through ${syntheticRef}, but GitHub did not return the fork repository. Plain git push may require manual remote configuration.`
				: undefined),
	};
}
...
async function resolvePrBranchSource(args: {
	...
	try {
		...
		return await fetchSyntheticPrBranch({
			...
			remoteName: args.remoteName,
			pr: args.pr,
			warning: `The PR head branch "${args.pr.headRefName}" was unavailable from ${args.remoteName}, so Velix fetched ${getSyntheticPrHeadRef(args.pr.number)} instead. Original error: ${err.originalError instanceof Error ? err.originalError.message : String(err.originalError)}`,
		});
	}
}
