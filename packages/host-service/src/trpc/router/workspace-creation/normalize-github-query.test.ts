...
describe("normalizeGitHubQuery — shared behaviors", () => {
	...
	describe("non-GitHub URLs (plain text fallback)", () => {
		...
		test("GitHub compare URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/compare/main...feature",
				repo,
				"pull",
			);
			expect(result.isDirectLookup).toBe(false);
...
describe("normalizeGitHubQuery — PR URLs", () => {
	describe("same repo", () => {
		test("basic URL", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3130",
				repo,
				"pull",
			);
			expect(result.query).toBe("3130");
		...
		test("/files tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/files",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/changes tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/changes",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/commits tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/commits",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("/checks tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/checks",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("trailing slash", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("query params", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354?diff=unified",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("query params on tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/files?diff=split&w=1",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("hash fragment", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354#discussion_r123",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("hash fragment on files tab", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/3354/files#diff-abc123",
				repo,
				"pull",
			);
			expect(result.query).toBe("3354");
		...
		test("case-insensitive owner/repo", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/pull/100",
				repo,
				"pull",
			);
...
describe("normalizeGitHubQuery — issue URLs", () => {
	describe("same repo", () => {
		...
		test("query params", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/issues/100?q=1",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
		...
		test("hash fragment (comment anchor)", () => {
			const result = normalizeGitHubQuery(
				"https://github.com/ezeslucky/velix/issues/100#issuecomment-12345",
				repo,
				"issue",
			);
			expect(result.query).toBe("100");
...
describe("normalizeGitHubQuery — cross-entity fallback", () => {
	test("issue URL pasted into PR search → plain text", () => {
		...
		expect(result.query).toBe("https://github.com/ezeslucky/velix/issues/100");
		expect(result.isDirectLookup).toBe(false);
		expect(result.repoMismatch).toBe(false);
	});
	...
	test("PR URL pasted into issue search → plain text", () => {
		...
		expect(result.query).toBe("https://github.com/ezeslucky/velix/pull/3354");
		expect(result.isDirectLookup).toBe(false);
		expect(result.repoMismatch).toBe(false);
	});
