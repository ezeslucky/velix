...
Host-service owns the lifecycle of `@velix/pty-daemon` — the long-lived
...
- **Singleton + bootstrap**: `src/daemon/singleton.ts` — process-level
  ...
  pattern below.
- **Manifest**: `src/daemon/manifest.ts` — `$VELIX_HOME_DIR/host/{orgId}/pty-daemon-manifest.json`.
...
The socket path lives in `os.tmpdir()/velix-ptyd-<sha256(orgId).slice(0,12)>.sock`
...
(`$VELIX_HOME_DIR/host/{orgId}/{host-service,pty-daemon}.log`)
...
Setting `VELIX_PTY_DAEMON_SOCKET` env var bypasses the supervisor in
