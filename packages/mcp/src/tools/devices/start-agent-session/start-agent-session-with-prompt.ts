...
export function registerPromptLaunchTool(server: McpServer) {
	server.registerTool(
		...
		{
			description:
				"Start an autonomous AI session in an existing workspace using a direct prompt instead of a task. Supports terminal agents and Velix. When paneId is provided, launch behavior is scoped to the tab containing that pane.",
