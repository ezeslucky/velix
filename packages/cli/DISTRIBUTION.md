...
How we ship `velix` as a standalone, downloadable bundle.

## Goals

1. User downloads a single tarball for their platform
2. `velix auth login` authenticates via browser (works headless too)
3. `velix start` runs the host service, connects to relay
4. The host machine is now accessible from any Velix client (web, mobile, desktop)
...
velix-darwin-arm64/
  bin/
    velix                # Bun-compiled CLI (single binary, no deps)
    velix-host           # Shell wrapper → exec node ../lib/host-service.js "$@"
  lib/
...
- **CLI** (`velix`): Pure JS — tRPC client, OAuth flow, config management. No native deps. Bun's `--compile` produces a single ~50MB binary.
- **Host service** (`velix-host`): Depends on `better-sqlite3` (C++ SQLite) and `node-pty` (C++ PTY). These are native Node.js addons that don't work in Bun. Must run on Node.

### How `start` works

```
velix start
  └─ reads ~/.velix/config.json (auth token, org ID, API URL)
  └─ resolves velix-host binary (sibling in bin/)
  └─ spawns: velix-host (which runs: lib/node lib/host-service.js)
  └─ passes env: AUTH_TOKEN, VELIX_API_URL, HOST_DB_PATH, RELAY_URL, etc.
...
velix start --daemon
  └─ same but detached, writes manifest to ~/.velix/host/<orgId>/manifest.json
...
```bash
# 1. Build CLI binary
bunx cli-framework build --target=bun-darwin-arm64 --outfile=dist/bin/velix
...
# 6. Write velix-host wrapper
cat > dist/bin/velix-host << 'EOF'
#!/bin/sh
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
exec "$SCRIPT_DIR/../lib/node" "$SCRIPT_DIR/../lib/host-service.js" "$@"
EOF
chmod +x dist/bin/velix-host

# 7. Package
tar -czf velix-darwin-arm64.tar.gz -C dist .
```
  ...
  build:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        with:
          name: velix-${{ matrix.target }}
          path: packages/cli/dist/velix-${{ matrix.target }}.tar.gz
```
...
```bash
curl -fsSL https://get.velix.sh | sh
```

Detects platform/arch, downloads tarball from GitHub Releases, extracts to `~/.velix/bin/`, prints PATH instructions.

### Manual

```bash
# Download
curl -LO https://github.com/user/velix/releases/latest/download/velix-darwin-arm64.tar.gz

# Extract
mkdir -p ~/.velix/bin
tar -xzf velix-darwin-arm64.tar.gz -C ~/.velix/bin

# Add to PATH
export PATH="$HOME/.velix/bin/bin:$PATH"
...
Publish `.deb` packages to a PPA for `apt-get install velix`.

### systemd / launchd (stretch goal)

```bash
velix host install   # writes systemd unit or launchd plist
...
| `packages/cli/src/lib/config.ts` | ~/.velix/config.json read/write |
