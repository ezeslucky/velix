...
import { CLIError, string } from "@velix/cli-framework";
...
export default command({
	description: "Authenticate with Velix. Re-run to switch organizations.",
	skipMiddleware: true,
	options: {
		...
		apiKey: string().desc(
			"Store a Velix API key (sk_live_…) at ~/.velix/config.json instead of running the OAuth flow",
		),
	},
	run: async (opts) => {
