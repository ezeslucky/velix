...
let testVelixHomeDir: string | null = null;
...
describe("ChatService OpenAI auth storage", () => {
	beforeEach(() => {
		...
		testVelixHomeDir = mkdtempSync(join(tmpdir(), "chat-service-test-"));
		process.env.VELIX_HOME_DIR = testVelixHomeDir;
	...
	afterEach(() => {
		if (testVelixHomeDir) {
			rmSync(testVelixHomeDir, { recursive: true, force: true });
			testVelixHomeDir = null;
		}
		if (originalVelixHomeDir) {
