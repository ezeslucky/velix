...
# shellcheck source=/dev/null
source "$VELIX_SCRIPT_DIR/lib/setup/steps.sh" # reuse allocate_port_base + helpers
...
local_allocate_ports() {
  ...
  LOCAL_DB_PROJECT="velix-$(sanitize_name "${VELIX_WORKSPACE_NAME:-$(basename "$PWD")}")"
...
local_write_env() {
  ...
  if [ -z "${VELIX_PORT_BASE:-}" ] || [ -z "$LOCAL_NEON_PROXY_PORT" ]; then
...

  {
    ...
    write_env_var "VELIX_WORKSPACE_NAME" "${VELIX_WORKSPACE_NAME:-$(basename "$PWD")}"
...
DEVVARS
  ...
  cat > "$VELIX_SCRIPT_DIR/ports.json" <<PORTSJSON
{
  "ports": [
...
local_write_config_overlay() {
  echo "🔧 Writing .velix/config.local.json (untracked overlay)..."
  cat > "$VELIX_SCRIPT_DIR/config.local.json" <<'CONFIGLOCAL'
{
  "setup": ["./.velix/setup.local.sh"],
  "teardown": ["./.velix/teardown.local.sh"]
}
CONFIGLOCAL
...
local_setup_main() {
  ...
  echo "🚀 Setting up Velix for LOCAL development..."
  echo ""
