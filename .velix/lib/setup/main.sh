...
setup_main() {
  ...
  echo "🚀 Setting up Velix workspace..."
  echo ""
  ...
  # Step 4: Seed local DB into Velix-dev-data/
  if ! step_seed_local_db; then
    step_failed "Seed local DB"
  fi

  # Step 5: Seed host-service DBs into Velix-dev-data/host/
  if ! step_seed_host_dbs; then
    step_failed "Seed host-service DBs"
  fi

  # Step 6: Seed auth token into Velix-dev-data/
