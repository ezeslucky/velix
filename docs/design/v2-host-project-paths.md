...
2. Or auto-clones to a hardcoded path: `~/.velix/repos/{projectId}`
...
| `packages/host-service/src/trpc/router/workspace/workspace.ts` | Workspace creation — auto-clones to `~/.velix/repos/{projectId}` if missing |
...
workspace.create(projectId, name, branch)
  → local project exists? → YES → create worktree from repoPath
                           → NO  → auto-clone to ~/.velix/repos/{projectId}
