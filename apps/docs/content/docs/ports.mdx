...
Velix does not assign per-workspace port ranges. It discovers listening ports from processes running in each workspace and lets you manage them from the UI.
...
Create `.velix/ports.json` in your repository:
...
- `ports.json` only labels ports that Velix already detects as listening
...
**Tips:**
- Commit `.velix/ports.json` to share port labels with your team
- **Pro tip:** If you want deterministic per-workspace port ranges, implement it in setup/teardown scripts by reserving a range in a shared file (for example `~/.velix/port-allocations.json`) during setup and releasing it during teardown. See this repo's examples: [`.velix/setup.sh`](https://github.com/ezeslucky/velix/blob/main/.velix/setup.sh) and [`.velix/teardown.sh`](https://github.com/ezeslucky/velix/blob/main/.velix/teardown.sh).
