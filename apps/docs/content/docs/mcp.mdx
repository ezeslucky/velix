---
title: MCP Server
description: Connect AI agents directly to Velix via MCP
---

<Callout type="info">
This document covers **MCP v2**. The v1 server is deprecated but still available at `https://api.velix.sh/api/agent/mcp` for existing integrations.
</Callout>

## Overview

Velix provides an [MCP (Model Context Protocol)](https://modelcontextprotocol.io) server that lets AI agents interact with your tasks, workspaces, and devices programmatically.
...
```bash title="terminal"
claude mcp add velix --transport http https://api.velix.sh/api/v2/agent/mcp
```
</Tab>
<Tab value="Codex">
```bash title="terminal"
codex mcp add velix --url https://api.velix.sh/api/v2/agent/mcp
```
</Tab>
<Tab value="Gemini CLI">
```bash title="terminal"
gemini mcp add --transport http velix https://api.velix.sh/api/v2/agent/mcp
```
</Tab>
<Tab value="Open Code">
...
[2m│[0m  [36mhttps://api.velix.sh/api/v2/agent/mcp[0m
[2m│[0m
...
<Tab value="Amp CLI">
```bash title="terminal"
amp mcp add --workspace velix https://api.velix.sh/api/v2/agent/mcp
```
</Tab>
</Tabs>
...
```json title=".mcp.json"
{
  "mcpServers": {
    "velix": {
      "type": "http",
      "url": "https://api.velix.sh/api/v2/agent/mcp"
    }
  }
}
```
...
```json title="settings.json"
{
  "mcpServers": {
    "velix": {
      "httpUrl": "https://api.velix.sh/api/v2/agent/mcp"
    }
  }
}
```
