---
title: CLI Reference
description: Complete reference for Velix-line interface, including commands and flags.
---
...
```text
velix <command> [subcommand] [options]
```

Run `velix --help` for the top-level command list, or
`velix <command> --help` for any group.
...
| `--api-key <key>` | `VELIX_API_KEY` | Use an API key instead of stored OAuth login. |
| `--help`, `-h` | | Show help for the current command. |
| `--version`, `-v` | | Print `velix v<version>` and exit. |
...
<Command
	...
	options={[
		...
		{
			flag: "--api-key <key>",
			description: "Store a Velix API key (`sk_live_…`) at `~/.velix/config.json` instead of running the OAuth flow.",
		},
	]}
>
...
the key, stores it at `~/.velix/config.json`, and clears any prior
OAuth session.

```bash
velix auth login --api-key sk_live_…
velix auth login --api-key sk_live_… --organization acme
```
...
<Command name="velix auth logout">
...
<Command name="velix auth whoami">
...
Update the Velix CLI and host server binary.
...
>
Downloads the matching `velix-<platform>.tar.gz` from GitHub Releases
and atomically replaces the install root. Only available in built
binaries; running from `bun run dev` errors out.

```bash
velix update                     # upgrade to the latest release
velix update --check             # show current → target without installing
velix update --version 0.1.2     # install a specific version (up or down)
```
...
<HumanOutput>
```text
Updated 0.1.4 → 0.1.5 (/Users/you/.local/share/velix)
```
</HumanOutput>
...
<Command
	name="velix organization switch <idOrSlug>"
	args={[
...
```bash
velix organization switch acme
velix org switch org_…
```
</Command>

<Command
	name="velix organization members list"
	options={[
...
```bash
velix organization members list
velix org members list -s satya
```
</Command>
...
one for the same Git URL. Use [`projects setup`](#velix-projects-setup-id)
when adopting an existing project on a fresh machine.

```bash
velix projects create --name "my-app" --local --clone https://github.com/org/my-app.git --parent-dir ~/code
velix projects create --name "my-app" --local --import ~/code/my-app
```
</Command>

<Command
	name="velix projects setup <id>"
	args={[
...
record. Counterpart to [`create`](#velix-projects-create) for the
"new machine, repo already in the org" case.

Find the project ID via [`projects list`](#velix-projects-list).
...
```bash
# Pick an ID from `velix projects list`, then clone it onto this machine
velix projects setup 47c31b04-… --local --parent-dir ~/code

# Or register an existing local checkout
velix projects setup 47c31b04-… --local --import ~/code/my-app
```
</Command>
...
happens via `velix start` on each machine — there is no
...
responding, the CLI errors out and points you at `velix start` rather
than silently falling through to the cloud.

<Command
	name="velix workspaces list"
	options={[
...
<Command
	...
	options={[
		...
		{ flag: "--host <id>", description: "Target host machineId. Mutually exclusive with `--local`; one is required." },
		{ flag: "--local", description: "Target this machine. Mutually exclusive with `--host`; one is required." },
		{ flag: "--agent <preset|uuid|velix>", description: "Agent to spawn after creation." },
...
<Command
	name="velix workspaces delete <id...>"
	args={[
...
```bash
velix workspaces delete ws_a ws_b ws_c
```
</Command>

<Command
	name="velix workspaces update <id>"
	args={[
...
```bash
velix workspaces update ws_… --name "better-name"
velix workspaces update ws_… --task-id tsk_…
velix workspaces update ws_… --clear-task
```
</Command>

<Command
	name="velix workspaces open <id>"
	args={[
...
>
Open a workspace in the Velix desktop app.

```bash
velix workspaces open ws_…
velix workspaces open ws_… --print
```
</Command>
...
```bash
velix agents list --local
velix agents list --host host_…
```
</Command>

<Command
	name="velix agents run"
	options={[
		{ flag: "--workspace <id>", required: true, description: "Workspace UUID to run the agent in." },
		{ flag: "--agent <preset|uuid|velix>", required: true, description: "Agent preset id (e.g. `claude`, `codex`), HostAgentConfig instance UUID, or `velix`." },
...
```bash
velix agents run --workspace ws_… --agent claude --prompt "Audit the login flow"
velix agents run --workspace ws_… --agent codex --prompt "Review this diff" --attachment ./trace.log
```
</Command>
...
```bash
velix tasks list --assignee-me
velix t list -s "auth" --json
```
</Command>

<Command
	name="velix tasks get <idOrSlug>"
	args={[
...
```bash
velix tasks get tsk_…
velix tasks get fix-login-bug
```
</Command>

<Command
	name="velix tasks create"
	options={[
...
```bash
velix tasks create --title "Audit auth flow" --priority high
```
</Command>

<Command
	name="velix tasks update <idOrSlug>"
	args={[
		{ name: "idOrSlug", required: true, description: "Task ID or slug." },
	]}
	options={[
...
```bash
velix tasks update fix-login-bug --priority urgent
```
</Command>

<Command
	name="velix tasks delete <idOrSlug...>"
	args={[
...
```bash
velix tasks delete tsk_a tsk_b
```
</Command>

<Command
	name="velix tasks statuses list"
	output={`Array<{
  id: string;
  name: string;
  type: string;
...
[`automations prompt get`](#velix-automations-prompt-get-id) to read it.
Use [`automations logs`](#velix-automations-logs-id) for run history.
</Command>

<Command
	name="velix automations create"
	options={[
		...
		{ flag: "--agent <agent>", description: "Host agent presetId, instance UUID, or 'velix' for built-in chat. Default: claude." },
	]}
	output="Automation"
>
...
<Command
	name="velix automations update <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	options={[
		...
		{ flag: "--agent <agent>", description: "New host agent presetId, instance UUID, or 'velix'." },
...
"no change", not "clear". Use [`automations prompt get`](#velix-automations-prompt-get-id)
or [`automations prompt set`](#velix-automations-prompt-set-id) to read or
replace the prompt body.
</Command>

<Command
	name="velix automations prompt get <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	output="Markdown"
...
```bash
# Read to a file
velix automations prompt get aut_… > prompt.md

# Verify a push landed
velix automations prompt get aut_… | diff - ./prompt.md
```
</Command>

<Command
	name="velix automations prompt set <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	options={[
...
```bash
# Write from file
velix automations prompt set aut_… --from-file ./prompt.md

# Write from stdin
cat ./prompt.md | velix automations prompt set aut_… --from-file -
```
</Command>

<Command
	name="velix automations delete <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	output={`{ deleted: string }`}
...
```bash
velix automations delete aut_…
```
</Command>

<Command
	name="velix automations pause <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	output="Automation"
...
```bash
velix automations pause aut_…
```
</Command>

<Command
	name="velix automations resume <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	output="Automation"
...
```bash
velix automations resume aut_…
```
</Command>

<Command
	name="velix automations run <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	output={`{
...
<Command
	name="velix automations logs <id>"
	args={[
		{ name: "id", required: true, description: "Automation ID." },
	]}
	options={[
...
`GEMINI_CLI`, `VELIX_AGENT`, or `CI` is set to a non-empty value, output
