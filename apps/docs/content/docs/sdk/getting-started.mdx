...
description: Drive Velix programmatically from any TypeScript or JavaScript codebase.
---

<Callout type="warn">
The SDK is in early alpha — it is not meant for production use and may be removed in the future. Stay on the latest version (`@velix_sh/sdk@latest`) while we iterate.
</Callout>

The Velix TypeScript SDK lets you create and manage your workspaces, tasks, projects, and automations from code — anything you can do in the dashboard, your scripts and services can do too.
...
<Tab value="pnpm">
```bash
pnpm add @velix_sh/sdk
```
</Tab>
<Tab value="yarn">
```bash
yarn add @velix_sh/sdk
```
</Tab>
<Tab value="bun">
```bash
bun add @velix_sh/sdk
```
</Tab>
</Tabs>
...
1. Open the [Velix desktop app](https://velix.sh/download) → **Settings → API Keys** → **Create**. The key starts with `sk_live_…` — copy it now (you won't be able to see it again).
2. Note your **organization ID** — find it in the org switcher inside the desktop app.

```bash
export VELIX_API_KEY=sk_live_…
export VELIX_ORGANIZATION_ID=…
```
...
```ts
import Velix, { APIError, NotFoundError, RateLimitError } from '@velix_sh/sdk';
...
try {
  ...
  if (err instanceof RateLimitError) {
    ...
    console.error(`Velix returned ${err.status}:`, err.error);
  }
}
```
