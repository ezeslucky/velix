...
We're excited to introduce Velix - a new way to work with AI coding agents.
...
Velix lets you run **10+ parallel coding agents** on your machine. Here's what that means for your workflow:
...
Velix uses isolated workspaces powered by git worktrees. Each task runs in its own environment, with its own terminal, its own files, and its own agent context.

![Velix workspace interface](/blog/introducing-velix/workspace-interface.png)

```bash
# Create a new workspace for a feature
velix create --branch feature/auth

# Your agent works in isolation
# Meanwhile, start another task...
velix create --branch feature/dashboard
```
