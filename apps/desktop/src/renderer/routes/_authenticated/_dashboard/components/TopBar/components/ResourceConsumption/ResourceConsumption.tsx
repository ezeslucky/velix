					...
					<>
						<div className="mt-3 grid grid-cols-3 divide-x divide-border/50">
							<MetricBadge
								...
								tooltip="Sum of CPU used by Velix and monitored terminal process trees. Over 100% means multiple CPU cores are busy. Sustained high values usually cause UI sluggishness and higher battery drain."
							/>
							<MetricBadge
								label="Memory"
								value={formatMemory(normalizedSnapshot.totalMemory)}
								tooltip="Resident memory used by Velix and monitored terminal process trees. If this keeps climbing without dropping, a workspace process may be retaining memory. High values increase swap risk and can cause stutter."
							/>
							<MetricBadge
								label="RAM Share"
								value={formatPercent(trackedMemorySharePercent)}
								tooltip="Percent of total system RAM used by monitored Velix resources only (not all apps). A high share means Velix is a major contributor to system memory pressure; a low share means pressure is likely elsewhere."
							/>
						</div>
						<Tooltip delayDuration={150}>
							<TooltipTrigger asChild>
