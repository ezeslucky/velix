...
if (IS_DEV) {
	...
	if (workspaceName) {
		app.setName(`Velix (${workspaceName})`);
	}
}
...
async function processDeepLink(url: string): Promise<void> {
	...
	// e.g. velix://tasks/my-slug -> /tasks/my-slug
...
if (!gotTheLock) {
	...
	(async () => {
		...
		protocol.handle("velix-icon", iconProtocolHandler);
		session
			.fromPartition("persist:velix")
			.protocol.handle("velix-icon", iconProtocolHandler);
		...
		if (process.platform === "darwin") {
			...
			protocol.handle("velix-font", fontProtocolHandler);
			session
				.fromPartition("persist:velix")
				.protocol.handle("velix-font", fontProtocolHandler);
		}
