{{MARKER}}
/**
 * VELIX Notification Plugin for OpenCode
 *
 ...
 * - Debug logging: Set VELIX_DEBUG=1 to enable verbose logging
 *
 * SUBAGENT FILTERING:
 ...
 */
export const VelixNotifyPlugin = async ({ $, client }) => {
  if (globalThis.__velixOpencodeNotifyPluginV8) return {};
  globalThis.__velixOpencodeNotifyPluginV8 = true;

  // Only run inside a v2 VELIX terminal session.
  if (!process?.env?.VELIX_TERMINAL_ID) return {};

  const notifyPath = "{{NOTIFY_PATH}}";
  const debug = process?.env?.VELIX_DEBUG === '1';
  ...
  const log = (...args) => {
    if (debug) console.log('[velix-plugin]', ...args);
  };

  /**
   * Sends a notification to VELIX's notification server.
  ...
  const notify = async (hookEventName) => {
    ...
    try {
      // VELIX_AGENT_ID=opencode is exported by the opencode wrapper and
