...
const COPILOT_HOOK_SIGNATURE = "# Velix copilot hook";
...
# Auto-inject Velix notification hooks when running inside a v2 Velix terminal.
if [ -n "$VELIX_TERMINAL_ID" ] && [ -f "${hookScriptPath}" ]; then
  COPILOT_HOOKS_DIR=".github/hooks"
  COPILOT_HOOK_FILE="$COPILOT_HOOKS_DIR/velix-notify.json"
  ...
  if [ -d ".git/info" ]; then
    grep -qF ".github/hooks/velix-notify.json" ".git/info/exclude" 2>/dev/null || \\
      printf '%s\\n' ".github/hooks/velix-notify.json" >> ".git/info/exclude" 2>/dev/null
  fi
fi
