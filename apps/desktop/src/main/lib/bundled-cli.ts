...
export const BUNDLED_CLI_SHIM_MARKER = "# Velix bundled CLI shim v1";
...
export function getBundledCliBinaryName(
	...
	return platform === "win32" ? "velix.exe" : "velix";
}

export function getBundledCliShimName(
	platform: NodeJS.Platform = process.platform,
): string {
	return platform === "win32" ? "velix.cmd" : "velix";
}
...
export function installBundledCliShim(
	...
	console.log(`[bundled-cli] Installed Velix CLI shim at ${shimPath}`);
	return "installed";
}
