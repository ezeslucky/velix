...
const TEST_DIR = join(tmpdir(), `velix-test-loader-${process.pid}`);
const WORKTREE_PATH = join(TEST_DIR, "worktree");
const VELIX_DIR = join(WORKTREE_PATH, ".velix");
const PORTS_FILE = join(VELIX_DIR, "ports.json");

describe("loadStaticPorts", () => {
	beforeEach(() => {
		mkdirSync(VELIX_DIR, { recursive: true });
	});
