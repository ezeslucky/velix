...
import {
	VELIX_HOME_DIR,
	VELIX_HOME_DIR_MODE,
	VELIX_SENSITIVE_FILE_MODE,
} from "./app-environment";

const RINGTONES_ASSETS_DIR = join(VELIX_HOME_DIR, "assets", "ringtones");
...
function writeCustomRingtoneMetadata(name: string): void {
	...
	try {
		chmodSync(CUSTOM_RINGTONE_METADATA_PATH, VELIX_SENSITIVE_FILE_MODE);
	} catch {
		// Best effort only.
	}
}
...
export function ensureCustomRingtonesDir(): void {
	...
	try {
		chmodSync(RINGTONES_ASSETS_DIR, VELIX_HOME_DIR_MODE);
	} catch {
		// Best effort only.
	}
}
...
export async function importCustomRingtoneFromPath(
	...
	if (areSamePath(sourcePath, destinationPath) && existsSync(destinationPath)) {
		try {
			chmodSync(destinationPath, VELIX_SENSITIVE_FILE_MODE);
		} catch {
			// Best effort only.
		}
