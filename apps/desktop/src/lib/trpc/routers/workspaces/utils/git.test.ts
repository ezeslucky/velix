...
describe("getDefaultBranch", () => {
	...
	function createIsolatedTestRepo(testName: string): {
		...
		const testDir = join(
			realpathSync(tmpdir()),
			`velix-test-${testName}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
...
describe("Shell Environment", () => {
	...
	test("getShellEnvironment captures .zshrc variables (requires -ilc)", async () => {
		...
		writeFileSync(
			join(tmpDir, ".zshrc"),
			'export __VELIX_SHELL_ENV_TEST__="interactive"\n',
		);
		...
		try {
			const env = await getShellEnvironment();
			expect(env.__VELIX_SHELL_ENV_TEST__).toBe("interactive");
		} finally {
...
describe("getCurrentBranch", () => {
	test("returns branch name for empty repo with unborn HEAD", async () => {
		const repoPath = join(
			realpathSync(tmpdir()),
			`velix-test-current-branch-empty-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	test("returns null in detached HEAD state", async () => {
		const repoPath = join(
			realpathSync(tmpdir()),
			`velix-test-current-branch-detached-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
...
describe("parsePrUrl", () => {
	...
	test("parses GitHub URL without protocol", () => {
		expect(parsePrUrl("github.com/ezeslucky/velix/pull/1781")).toEqual({
			owner: "ezeslucky",
			repo: "velix",
			number: 1781,
		});
