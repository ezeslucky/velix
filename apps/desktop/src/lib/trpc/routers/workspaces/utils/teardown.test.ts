...
import { PROJECTS_DIR_NAME, VELIX_DIR_NAME } from "shared/constants";

const TEST_DIR = join(tmpdir(), `velix-test-teardown-${process.pid}`);
const TEST_VELIX_HOME = join(TEST_DIR, "velix-home");
...
const ORIGINAL_VELIX_HOME_DIR = process.env.VELIX_HOME_DIR;
...
describe("runTeardown", () => {
	beforeEach(() => {
		process.env.VELIX_HOME_DIR = TEST_VELIX_HOME;
		// Create test directories
		mkdirSync(join(MAIN_REPO, ".velix"), { recursive: true });
	...
	afterEach(() => {
		...
		if (ORIGINAL_VELIX_HOME_DIR === undefined) {
			delete process.env.VELIX_HOME_DIR;
		} else {
			process.env.VELIX_HOME_DIR = ORIGINAL_VELIX_HOME_DIR;
		}
	...
	test("returns success when config has no teardown commands", async () => {
		writeFileSync(
			join(MAIN_REPO, ".velix", "config.json"),
	...
	test("uses worktreePath config when present", async () => {
		...
		mkdirSync(join(WORKTREE, ".velix"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".velix", "config.json"),
	...
	test("sets environment variables for teardown scripts", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				teardown: [
					`echo "$VELIX_WORKSPACE_NAME|$VELIX_ROOT_PATH" > "${envFile}"`,
				],
			}),
		);
	...
	test("uses managed wrapper path for teardown managed binaries even with PATH rewrites", async () => {
		...
		const wrapperBinDir = join(TEST_VELIX_HOME, "bin");
