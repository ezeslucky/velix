...
const TEST_DIR = join(tmpdir(), `velix-test-setup-${process.pid}`);
...
describe("loadSetupConfig", () => {
	...
	test("returns null for invalid JSON", () => {
		writeFileSync(join(MAIN_REPO, ".velix", "config.json"), "{ invalid json");
	...
	test("prefers worktree config over main repo config", () => {
		const mainConfig = { setup: ["./.velix/setup.sh"] };
...
describe("config.local.json", () => {
	...
	test("local config with before/after merges with base setup", () => {
		...
		writeFileSync(
			join(MAIN_REPO, ".velix", "config.local.json"),
			JSON.stringify({
	...
	test("worktree local config takes priority over main repo local config", () => {
		...
		mkdirSync(join(WORKTREE, ".velix"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".velix", "config.local.json"),
