...
describe("shell env cache", () => {
	test("getShellEnvironment forceRefresh bypasses cached values", async () => {
		...
		writeFileSync(
			zshrcPath,
			'export __VELIX_SHELL_ENV_REFRESH_TEST__="first"\n',
		);
		...
		try {
			...
			expect(cachedEnv.__VELIX_SHELL_ENV_REFRESH_TEST__).toBe("first");

			writeFileSync(
				zshrcPath,
				'export __VELIX_SHELL_ENV_REFRESH_TEST__="second"\n',
			);

			const stillCachedEnv = await getShellEnvironment();
			expect(stillCachedEnv.__VELIX_SHELL_ENV_REFRESH_TEST__).toBe("first");

			const refreshedEnv = await getShellEnvironment({ forceRefresh: true });
			expect(refreshedEnv.__VELIX_SHELL_ENV_REFRESH_TEST__).toBe("second");
		} finally {
