...
- `BuiltinAgentId` — `"claude" | "amp" | "codex" | "gemini" | "mastracode" | "opencode" | "pi" | "copilot" | "cursor-agent" | "velix"`. Defined in `packages/shared/src/agent-catalog.ts:23` from `BUILTIN_TERMINAL_AGENTS` in `packages/shared/src/builtin-terminal-agents.ts:59`.
...
| Step | Where |
| --- | --- |
| `VELIX_TERMINAL_ID` + hook URL injected into PTY env | `packages/host-service/src/terminal/env.ts:183,195` |
...
```ts
import type { BuiltinAgentId, AgentDefinitionId } from "@velix/shared";
...
```sh
VELIX_AGENT_ID=claude $VELIX_HOME_DIR/hooks/notify.sh
```
...
`VELIX_AGENT_ID` is the only env var the wrapper needs to set. `sessionId` is parsed out of the agent's own JSON payload at hook time (next step).
...
PAYLOAD="{\"json\":{
  \"terminalId\":\"$(json_escape "$VELIX_TERMINAL_ID")\",
  \"eventType\":\"$(json_escape "$EVENT_TYPE")\",
  \"agent\":{
    \"agentId\":\"$(json_escape "$VELIX_AGENT_ID")\",
...
Add `agent?: AgentIdentity` to `AgentLifecycleMessage` (`packages/host-service/src/events/types.ts:25`) and `AgentLifecyclePayload` in `@velix/workspace-client`. Define `AgentIdentity` once — best home is `packages/shared/src/agent-identity.ts` since `BuiltinAgentId` lives in `@velix/shared` already — and re-export from `@velix/workspace-client` so host-service / main / renderer share one source of truth.
...
`usePresetIcon` already handles dark/light variants and returns `undefined` for unknown ids → nothing renders. `BUILTIN_AGENT_LABELS` (from `@velix/shared/agent-catalog`) gives the human-readable name for tooltip / a11y. Safe default for any future agent id that ships before its icon does.
...
- Mutation check — flip `VELIX_AGENT_ID=claude` to empty in the wrapper template; renderer test asserting the icon renders should fail.
