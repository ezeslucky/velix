...
| Seed `.env` / `.velix/setup.json` | no | no | no |
...
2. **No setup-script execution.** `loadSetupConfig()` / `initialCommands` (used by `workspaces.create`) are not wired into `projects.cloneRepo`. Repos with `.velix/setup.json` are silently ignored at project-create time.
3. **No `.velix` config or env seeding.** Workspace creation copies `.velix` config into the worktree (`copyVelixConfigToWorktree`); project creation never bootstraps anything in the cloned repo.
...
The host service is the per-machine local Velix orchestrator (`LocalHostServiceProvider` wraps `_authenticated/layout.tsx:203`). Owns the canonical project model used by v2 cloud features.
