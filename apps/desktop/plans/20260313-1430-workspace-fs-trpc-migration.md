...
- `createDirectory` is idempotent — succeeds silently if directory already exists. `recursive?: boolean` enables `mkdir -p` semantics for higher-level callers such as nested new-file creation. Needed for infra setup (`.velix/` dir) without error handling.
...
1. `trpc.filesystem.createDirectory({ workspaceId, absolutePath: "<workspace>/.velix" })` — idempotent, safe if exists
2. `trpc.filesystem.writeFile({ workspaceId, absolutePath: "<workspace>/.velix/<file>", content })` — default create-or-overwrite
