...
Velix Desktop is an Electron app. In this repo:
...
This plan intentionally does not implement cloud terminals, but the abstraction boundary should be compatible with adding a backend that runs terminal sessions inside a remote “runner” (a background agent on a server) while preserving Velix Desktop concepts like worktrees, “changes” (diff/status), and agent lifecycle indicators.
...
2. **Agent lifecycle events assume localhost hooks:** terminal env injects `VELIX_*` and `VELIX_PORT` (`apps/desktop/src/main/lib/terminal/env.ts`), and the notify hook script `curl`s `http://127.0.0.1:$VELIX_PORT/hook/complete` (`apps/desktop/src/main/lib/agent-setup/templates/notify-hook.template.sh`). This cannot work from a remote runner.
...
1. `bun run lint`, `bun run typecheck --filter=@velix/desktop`, and `bun test --filter=@velix/desktop` all pass.
...
Run these commands from the repo root:
    ...
    bun run test --filter=@velix/desktop
...
- [x] Run `bun run typecheck --filter=@velix/desktop`
...
- [x] Run `bun test --filter=@velix/desktop`
