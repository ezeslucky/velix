...
1. A “workspace” is a Velix concept representing a git worktree; routes use `/workspace/$workspaceId`.
2. Tabs and panes are conceptually workspace-local. A tab should never render while viewing another workspace route.
3. UI state persistence is local-only (lowdb JSON) at `~/.velix/app-state.json` (see `apps/desktop/src/main/lib/app-environment.ts`), not the production database.
...
- `apps/desktop/src/main/lib/app-state/index.ts` loads and stores the lowdb file at `APP_STATE_PATH`, which is defined in `apps/desktop/src/main/lib/app-environment.ts` as `~/.velix/app-state.json`.
...
2. Update `apps/desktop/src/main/lib/app-state/index.ts` so `ensureValidShape` normalizes `data.tabsState` via `normalizeTabsState` (imported from the shared helper). This ensures old `~/.velix/app-state.json` files are reshaped on startup.
...
Acceptance:
    ...
    bun run typecheck --filter=@velix/desktop
...
Acceptance:
    ...
    bun run typecheck --filter=@velix/desktop
    bun run lint:check-node-imports --filter=@velix/desktop
...
2. Move the local lowdb file out of the way (do not delete immediately):
    ...
    mv ~/.velix/app-state.json ~/.velix/app-state.json.bak
