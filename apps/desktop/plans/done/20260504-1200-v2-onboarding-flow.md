...
…then signing out, clearing local-db (`rm -rf ~/Library/Application Support/velix/db.sqlite`), clearing the localStorage entry `v2-local-override-v2`, restarting, and walking through the five steps. Each completed step should persist so a force-close + reopen resumes at the right step.
...
- [x] (2026-05-04 13:30Z) Milestone 1: Onboarding store + route shell. Created Zustand store at `apps/desktop/src/renderer/stores/onboarding/` (persist key `velix-onboarding-v1`), shared `OnboardingProgress` UI, sub-layout, and 6 step page stubs.
...
- Create `apps/desktop/src/renderer/stores/onboarding/onboardingStore.ts` (Zustand + `devtools` + `persist`, persist key `velix-onboarding-v1`). State shape:
...
Reset to "fresh install" state for repeated verification:
    ...
    rm -rf "$HOME/Library/Application Support/velix/db.sqlite"
    # In DevTools console of the running Electron renderer:
    localStorage.removeItem("v2-local-override-v2");
    localStorage.removeItem("velix-onboarding-v1");
...
- Onboarding store mid-flow: call `reset()` from DevTools or delete `localStorage["velix-onboarding-v1"]`.
