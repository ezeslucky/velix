...
> design. Current behavior treats `.velix/ports.json` as supplemental label
...
1. The `.velix` directory already exists in repositories using Velix (same location as `config.json` for setup/teardown scripts).
2. Static ports should completely replace dynamic port discovery when `ports.json` is present (not merge with dynamic ports).
3. Each workspace reads from its own worktree's `.velix/ports.json` file (workspace-scoped, not project-scoped).
...
- Observation: Toast import path is `@velix/ui/sonner`, not `sonner` directly
...
- Observation: File watching needed additional logic to handle both file and directory watching
  Evidence: When ports.json doesn't exist, we watch the .velix directory for file creation; when it exists, we watch the file directly for changes

## Decision Log

- Decision: Workspace-scoped ports.json reading
  Rationale: User requested this approach. Each workspace's worktree has its own `.velix/ports.json`, allowing different branches to have different port configurations.
...
- Constants defined in `apps/desktop/src/shared/constants.ts`: `PROJECT_VELIX_DIR_NAME = ".velix"`, `CONFIG_FILE_NAME = "config.json"`
...
The app uses `sonner` for toast notifications. Import `toast` from `@velix/ui/sonner` and call `toast.error("message")` to show an error toast.
...
1. Constructs path: `join(worktreePath, PROJECT_VELIX_DIR_NAME, PORTS_FILE_NAME)`
...
3. **Overview section**: Explain that Velix normally auto-discovers ports from running processes, but you can override this with a static configuration file.

4. **Configuration section**: Show path `.velix/ports.json`
...
8. **Tips section**:
   ...
   - Share port documentation with your team by committing `.velix/ports.json`
...
For manual validation:
    ...
    # In the workspace's worktree, create .velix/ports.json:
...
4. **Manual test - no config**: With no `.velix/ports.json`, dynamic port discovery works as before
...
- `@velix/ui/sonner` for toast notifications (already used in app)
