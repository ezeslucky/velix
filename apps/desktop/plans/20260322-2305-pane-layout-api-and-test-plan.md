...
Define a shippable Velix pane-layout package before we continue implementation.
...
- Renderer layer must be design-system-native, using Tailwind + `@velix/ui`
...
- `react`
  ...
  - group/tab chrome built from `@velix/ui`
...
This package is internal to Velix. It does not need an external-consumer-neutral API.
...
New for Velix because cross-root drag/drop is a core requirement.
...
1. Lock the core API in `@velix/pane-layout`
2. Write reducer and tree utility tests first
3. Implement root-aware reducer
4. Implement minimal React renderer with `@velix/ui` primitives
...
- a Velix-specific React renderer
