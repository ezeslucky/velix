...
- [ ] **A2. Prompt + text attachment** — drag `v2-launch-test-artifacts/trace.log`
      into the modal. After launch: verify `.velix/attachments/trace.log`
      exists in the worktree (terminal: `ls .velix/attachments/`).
      Claude prompt contains `![trace.log](.velix/attachments/trace.log)`.
...
## B. Happy-path — chat agent (Velix Chat)

Disable Claude (or set Velix Chat as preferred via order in settings).
