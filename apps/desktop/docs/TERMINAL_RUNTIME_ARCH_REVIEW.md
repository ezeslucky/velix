...
Velix Desktop is an Electron app that provides:
...
Today, terminals can run locally and (optionally) persist via a background “terminal host” daemon. In the future, we want to support executing terminals in the cloud / on a remote runner while keeping the same “Velix UX primitives” (worktrees, changes/diff, agent status, etc.).
