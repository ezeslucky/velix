...
The relay is how remote desktop hosts proxy HTTP + WebSocket traffic to clients on the web. It's a Hono + `@hono/node-ws` server on fly (`velix-relay`, `sjc`) that accepts two kinds of connections:

1. **Host-side** — desktops open `wss://relay.velix.sh/tunnel?hostId=<id>&token=<jwt>`. Relay calls this a "tunnel" and keeps an in-memory `Map<hostId, TunnelState>` (`apps/relay/src/tunnel.ts:30`).
...
**Exit criteria:** `fly scale count 3 -a velix-relay`, then have three of us connect our desktops, and every client request succeeds regardless of which machine the client's HTTP lands on.
