...
export async function processAssistantMessage({
	...
	if (!activeSubscription) {
		...
		await slack.chat.postMessage({
			...
			text: "The Velix Slack integration requires a Pro plan.",
			blocks: [
				{
					type: "section",
					text: {
					...
					elements: [
						{
							...
							url: "https://app.velix.sh/settings/billing",
							style: "primary",
						},
					],
				},
	...
	if (!slackUserLink) {
		...
		await slack.chat.postMessage({
			channel: event.channel,
			thread_ts: event.thread_ts ?? event.ts,
			text: "To use Velix, you need to link your Slack account first.",
			blocks: [
				{
					type: "section",
					text: {
