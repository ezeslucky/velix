---
description: Create a task and assign it to me via Velix MCP
allowed-tools: mcp__velix__create_task, mcp__velix__list_members, Bash(git config user.email)
---

Create a new task in Velix and assign it to me.
...
4. Resolve the current user's member ID by calling `mcp__velix__list_members` and matching against the git user (run `git config user.email` to get the current user's email)
5. Create the task using `mcp__velix__create_task` with:
