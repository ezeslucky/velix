...
allowed-tools: mcp__velix__create_task, mcp__velix__list_members, mcp__velix__list_devices, mcp__velix__list_projects, mcp__velix__create_workspace, mcp__velix__start_agent_session, Bash(git config user.email)
---

Create a new task in Velix, spin up a workspace, and start an AI agent session to work on it.
...
- Call `mcp__velix__list_members` and match against the git user email (`git config user.email`) to get the current user's member ID
- Call `mcp__velix__list_devices` and select the device owned by the current user
- Call `mcp__velix__list_projects` on the resolved device and select the project matching the current git repo
...
- Create the task using `mcp__velix__create_task` with:
...
- Create the workspace using `mcp__velix__create_workspace`

### 3. Start AI agent session

- Start an AI agent session using `mcp__velix__start_agent_session` with the created task ID and workspace ID
