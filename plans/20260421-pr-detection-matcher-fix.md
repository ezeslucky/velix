...
Two workspace → PR match sites keyed on branch name alone. Any cross-fork PR whose `headRefName` collided with a local branch got attached — e.g. PR #3261 (`quueli/velix-windows:main` → `ezeslucky/velix:main`) showing on every local `main` workspace.
...
- PR #3261 (cross-fork): workspace upstream `ezeslucky/velix#main` vs PR key `quueli/velix-windows#main` — no match in v2. v1 predicate returns false via `isCrossRepository` check. ✓
